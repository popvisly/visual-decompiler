    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {boards.map((board: any) => (
                <div
                    key={board.id}
                    className="group bg-white p-10 rounded-[3rem] border border-[#E7DED1] hover:border-[#BB9E7B] transition-all flex flex-col h-full shadow-[0_20px_60px_rgba(20,20,20,0.02)] hover:shadow-[0_40px_100px_rgba(187,158,123,0.1)] hover:-translate-y-1 relative"
                >
                    <Link href={`/dashboard/boards/${board.id}`} className="absolute inset-0 z-10 rounded-[3rem]" aria-label="View collection" />

                    <div className="flex justify-between items-start mb-10 relative z-20 pointer-events-none">
                        ...
                        <div className="flex items-center gap-3 pointer-events-auto">
                            <span className="text-[10px] font-bold text-[#6B6B6B] uppercase tracking-[0.3em] bg-[#FBF7EF] px-4 py-2 rounded-full border border-[#E7DED1] shadow-sm">
                                {board.board_items?.[0]?.count || 0} Assets
                            </span>
                            <DeleteBoardCardButton boardId={board.id} />
                        </div>
                    </div>

                    <h3 className="text-2xl font-light text-[#141414] uppercase tracking-tight mb-4 group-hover:text-[#BB9E7B] transition-colors leading-[0.9] relative z-20 pointer-events-none">
                        {board.name}
                    </h3>
                    <p className="text-[11px] text-[#6B6B6B] font-medium leading-relaxed line-clamp-2 mb-10 flex-1 uppercase tracking-tight opacity-70 relative z-20 pointer-events-none">
                        {board.description || 'Deconstructing a strategic pattern collection.'}
                    </p>

                    <div className="flex items-center gap-3 text-[10px] font-bold text-[#141414] uppercase tracking-[0.3em] mt-auto opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0 relative z-20 pointer-events-none">
