...
export default function DeconstructionModule() {
    const bars = [40, 65, 30, 80, 45, 90, 60, 35, 75, 50, 85, 40, 70, 55, 95, 60, 45, 80];
    return (
        <div className="my-12 p-6 md:p-10 rounded-[2rem] bg-[#101010] border border-[#8B6A3D]/5 shadow-inner">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-[#D4A574]/10 flex items-center justify-center">
                        <BrainCircuit className="w-4 h-4 text-[#D4A574]" />
                    </div>
                    <div>
                        <h4 className="text-[10px] font-bold text-[#D4A574] uppercase tracking-widest">Live Module</h4>
            ...
            <div className="relative h-48 rounded-xl bg-gradient-to-r from-[#141414] to-[#1a1a1a] border border-[#8B6A3D]/5 overflow-hidden flex items-end px-4 gap-1">
                ...
                {bars.map((height, i) => (
                    <div key={i} className="flex-1 flex flex-col justify-end gap-2 group relative z-10">
                        <div
                            className="w-full bg-[#D4A574]/35 group-hover:bg-[#D4A574]/75 transition-colors rounded-t-sm"
                            style={{ height: `${height}%` }}
                        />
                    </div>
                ))}
            ...
            <div className="mt-3 flex items-center justify-between text-[9px] uppercase font-bold tracking-widest text-white/20">
                <span>Low</span>
