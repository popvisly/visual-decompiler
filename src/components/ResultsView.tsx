        ...
        <div className="w-full max-w-6xl mx-auto page-enter">
            ...
            <div className={`relative grid grid-cols-1 gap-8 ${showMedia ? 'lg:grid-cols-5' : ''}`}>
                ...
                <div className={`${showMedia ? 'lg:col-span-3' : ''} space-y-6`}>
                    {/* ── Executive Summary — Condensed Key Metrics ── */}
                    <ResultsCard title="Executive Summary" variant="strategy">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-2">
                                <ForensicTooltip term="Market Resonance" definition="A quantitative measure of how well the creative DNA aligns with current high-performing industry benchmarks and cultural sentiment within a specific category.">
                                    <p className="text-[9px] font-bold text-[#6B6B6B] uppercase tracking-widest">Market Resonance</p>
                                </ForensicTooltip>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-4xl font-light text-[#141414]">{(roiPredict?.score || 70)}%</span>
                                    <span className={`text-[10px] font-bold uppercase ${(roiPredict?.score || 70) > 60 ? 'text-green-600' : 'text-red-600'}`}>
                                        {(roiPredict?.score || 70) > 60 ? 'High' : 'Moderate'}
                                    </span>
                                </div>
                                <p className="text-[11px] text-[#6B6B6B] leading-tight">{roiPredict?.rationale || 'Calculating resonance score...'}</p>
                            </div>

                            {/* Category Density */}
                            <div className="space-y-2">
                                <p className="text-[9px] font-bold text-[#6B6B6B] uppercase tracking-widest">Category Density</p>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-4xl font-light text-[#141414]">{forecasting?.saturationLevel || 0}%</span>
                                    <span className={`text-[10px] font-bold uppercase ${(forecasting?.saturationLevel || 0) > 60 ? 'text-red-600' : 'text-green-600'}`}>
                                        {(forecasting?.saturationLevel || 0) > 60 ? 'Saturated' : 'Open'}
                                    </span>
                                </div>
                            ...
                            <div className="space-y-2">
                                <ForensicTooltip term="Tactical Window" definition="The estimated lifespan of the ad's effectiveness before 'Creative Fatigue' sets in, requiring a strategic pivot or asset refresh.">
                                    <p className="text-[9px] font-bold text-[#6B6B6B] uppercase tracking-widest">Tactical Window</p>
                                </ForensicTooltip>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-4xl font-light text-[#141414]">{forecasting?.estimatedLifespanDays || 0}</span>
                                    <span className="text-[10px] font-bold text-[#6B6B6B] uppercase">Days</span>
                                </div>
                                <p className="text-[11px] text-[#6B6B6B] leading-tight">Estimated effectiveness period</p>
                            </div>
                        </div>
                            ...
                            <div className="mt-6 pt-6 border-t border-[#E7DED1]">
                                <div className="bg-accent/10 border border-accent/20 rounded-2xl p-5 flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[13px] text-[#141414] font-medium leading-relaxed italic">{anomalyReason}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    {/* ── Objection Dismantling + Key Strategy ── */}
                    <BlurGate locked={isLimited}>
                        <ResultsCard title="Strategic Intelligence" variant="strategy" tooltip="The core persuasion framework - what objection is being tackled and how the ad positions itself in the market.">
                            <div className="space-y-6">
                                {/* Objection Dismantling - PRIMARY */}
                                {strat?.objection_tackle && (
                                    <div>
                                        <ForensicTooltip term="Objection Dismantling Logic" definition="A forensic breakdown of how the creative specifically neutralizes common consumer hesitations (e.g., price, effort, or trust) through subtle visual cues and messaging.">
                                            <p className="text-[10px] font-bold uppercase tracking-[0.15em] mb-2 text-[#141414]">
                                                Objection Dismantling Logic
                                            </p>
                                        </ForensicTooltip>
                                        <p className="text-lg font-light text-[#141414] leading-relaxed italic border-l-[3px] border-accent pl-4">
                                            "{strat.objection_tackle}"
                                        </p>
                                    </div>
                                )}
                                ...
                                {/* Key Strategy Fields */}
                                <div className="grid grid-cols-1 gap-y-5 pt-4">
